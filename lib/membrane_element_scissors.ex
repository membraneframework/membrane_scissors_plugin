defmodule Membrane.Element.Scissors do
  ...
  defp cut(%{next_cuts: [_cut0, {from, _to}], time: time} = state) when time >= from do
    state |> next_cut() |> cut()
  end

  defp cut(%{next_cuts: [{from, _to} | _], time: time} = state) when time < from do
    {true, state}
  end
  ...
  defp cut(%{next_cuts: [{_from, to} | _], time: time} = state)
       when time < to do
    {false, state}
  end
